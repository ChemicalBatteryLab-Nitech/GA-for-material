...
_ce_spec = _ilu.spec_from_file_location(
    "calc_engine",
    os.path.abspath(os.path.join("Specific", inp_POSCAR.calc_engine))
)
calc_engine = _ilu.module_from_spec(_ce_spec)
_ce_spec.loader.exec_module(calc_engine)
del _ilu, _ce_spec
...
if mlga == True:
    ...
    save_ml_log = inp_ga.save_ml_log
    MLoriginal = inp_ga.MLoriginal
    from SpecificML import ml_engine
#For GA
...
def make_time_file():
    if generation_index == 0:
        ...
        if mlga == True:
            with open("ml_time", "w") as q:
                q.write("# generation  make_candidate_gene  make_model  prediction\n")
    ...
    if mlga == True:
        with open("ml_time", "a") as p:
            p.write(f"{generation_index} {mlgen_time}  {mkmodel_time} {pred_time}\n")
...
class Indivisual:
    ...
    def calc_score(self):
        ...
        print("start calc")
        score = calc_engine.run(self.cpwd)
        print("calc finish")
        self.make_savefiles()
...
def create_first_generation(POPULATION, genoms):
    ...
    futures = []
    with ProcessPoolExecutor(max_workers=n_parallel) as tpe:
        for i in generations:
            futures.append(tpe.submit(i.calc_score))
    for n, fut in enumerate(futures):
        generations[n].score = fut.result()
    return generations
...
def ga(generation):
    ...
    global mlgen_time, mkmodel_time, pred_time
    best_score = []
    best = []
    tmp_answer = 0
    ...
    for i in range(start_generation, GENERATION):
        ...
        #train ML model from evaluated generation (in-memory, no file I/O)
        mkmodel_start = time.time()
        if mlga == True:
            _genes     = [ind.genom for ind in generation]
            _energies  = [ind.score for ind in generation]
            ml_model, ml_rmse = ml_engine.train(_genes, _energies)
            if save_ml_log:
                with open(outMLreg, "a") as _f:
                    _f.write(f"{generation_index} {ml_rmse}\n")
        ...
        if mlga == True:
            ...
            ml_mu  = mutate(selected, [], "ml_mu")
            mlgen_time = time.time() - mlgen_start

            #predict on ML candidates and select top-N per mode (in-memory)
            pred_start = time.time()
            ml_top_genes = []
            for mlmode in mllist:
                ml_data   = locals()[f"ml_{mlmode}"]
                cand_genes = [d[0] for d in ml_data]
                sorted_idx = ml_engine.predict(ml_model, cand_genes)
                n_select   = globals()[f"n_ml{mlmode}"]
                for j in sorted_idx[:n_select]:
                    d = ml_data[j]
                    g, k = d[0], d[2]
                    index_indiv += 1
                    children.append([g, "offsprings", f"ml_{k}", index_indiv])
                    all_gen.append(g)
                    make_genestock(g)
                    ml_top_genes.append(g)
            if save_ml_log:
                with open(outMLpred, "a") as _f:
                    for g in ml_top_genes:
                        _f.write(" ".join(g) + "\n")
